...
def live_pushups():
    ...
    # Percentage values of bar_left and bar_right
            ...
            if (per_val_langle-per_val_rangle)>10:
                ...
                engine.say("Right Hand is too low , cannot count the rep")
                engine.runAndWait()
            elif(per_val_rangle-per_val_langle)>10:
                print("Left Hand is too low , cannot count the rep")
                engine.say("Left Hand is too low , cannot count the rep")
                engine.runAndWait()
            else:
                print("Correct")
                engine.say("Correct")
...
def main():
    ...
    st.text("**Real-time Push-up Detection with MediaPipe**")
    live_pushups()
